...
function WIN.FileLabelLog.button:onClick()
  local file = ui.opendialog()

  if file ~= nil then
    if ui.confirm("Are you sure you want to override the file?") == "yes" then
...
function WIN.ButtonStart:onClick()
  ...
  local file = WIN.FileLabelLog.text
  ...
  if COM:open(state) then
    ...
    if not string.isempty(WIN.EditOutput.text) then
      WIN.EditOutput:save(file)
    end
  end
...
function WIN:onResize()
  ...
  self.GM_BOTTOM:update()

  self.WM_RESIZE:change("width", self.width - 20)
end
